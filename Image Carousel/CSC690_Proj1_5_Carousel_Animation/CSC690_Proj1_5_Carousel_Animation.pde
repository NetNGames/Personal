...
float thumbRatio;
int fullWidth = 800;
int windowWidth = fullWidth/5; //Makes room for 5 images
int fullHeight = 600;
int windowHeight = fullHeight/4; //So tall, skinny images gets scaled down
int border = 50;
...
void drawImage(int index, int offset) {
  ...
  if (index==selectedImage) { //Draws border around selected image
    selectBorder(imageList[index], offset);
  }
}
void selectBorder(PImage image, int offset) {
  ...
  noFill();
  //http://processing.org/reference/rect_.html
  rect((windowWidth*offset)+(windowWidth/2), fullHeight/2, windowWidth, windowHeight);
}
